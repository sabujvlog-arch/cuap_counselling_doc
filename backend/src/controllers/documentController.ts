...
export const uploadDocument = async (req: AuthRequest, res: Response) => {
  ...
  if (!studentId || !category || !fileName || !fileData) {
    return res
      .status(400)
      .json({ error: 'studentId, category, fileName, and fileData (base64) are required.' });
  }
  ...
  try {
    ...
    if (req.user.role === 'student') {
      ...
      if (studentProfile.rows.length === 0 || studentProfile.rows[0].id !== parseInt(studentId)) {
        return res
          .status(403)
          .json({ error: 'Forbidden. Students can only upload for themselves.' });
      }
    }
    ...
    await query(
      ...
      [
        req.user.id,
        'UPLOAD_DOCUMENT',
        `Uploaded document ${fileName} (v${finalVersion}) for Student ID ${studentId}`,
        req.ip,
      ],
    );
  ...
  try {
    ...
    await query(
      ...
      [
        req.user.id,
        'SIGN_CONSENT',
        `Signed digital counseling consent form. Signature: ${signature}`,
        req.ip,
      ],
    );

    return res.json({ message: 'Consent form signed and submitted successfully.' });
  } catch (err) {
    console.error('Sign consent form error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
};
