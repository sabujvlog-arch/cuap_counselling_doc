...
export const aiAssist = async (req: AuthRequest, res: Response) => {
  ...
  if (!req.user || req.user.role !== 'provider') {
    return res
      .status(403)
      .json({ error: 'Clinical assistant features are only accessible by authorized providers' });
  }
  ...
  try {
    ...
    if (type === 'suggest-diagnosis') {
      ...
      return res.json({
        ...
        referenceNote:
          'Suggestions generated by NLP symptom mapping (DSM-5-TR reference only. Provider clinical verification required before locking).',
      });
    }
    ...
    if (type === 'suggest-treatment') {
      ...
      if (diagLower.includes('depress')) {
        ...
        interventions = [
          ...
          'Socratic questioning to dispute negative self-beliefs',
        ];
        ...
        interventions = [
          'Box Breathing exercises (4-4-4-4)',
      ...
      for (const row of sessionsRes.rows) {
        ...
        if (
          ...
          matchingSessions.push({
            ...
            complaintSnippet:
              decryptedComplaint.substring(0, 100) + (decryptedComplaint.length > 100 ? '...' : ''),
          });
        }
      }
